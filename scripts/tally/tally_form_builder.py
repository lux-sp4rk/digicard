...
def generate_group_uuid():
    """Generate a groupUuid (appears to be a hex-like pattern, using UUID for consistency)."""
    return str(uuid.uuid4())
...
def build_dropdown_block(options):
    """Create a DROPDOWN block with options."""
    return {
        "uuid": generate_uuid(),
        "type": "DROPDOWN",
        "groupUuid": generate_group_uuid(),
    ...
    with open(spec_file) as f:
        for line in f:
            ...
            if item_type == "FORM_TITLE":
                ...
                blocks.append(build_dropdown_block(payload.get("options", [])))
